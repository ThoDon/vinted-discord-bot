...
const recreate = () => {
  console.log(`💎 Creating local Docker dependency...`);

  exec(
    `docker build --pull --rm -f "Dockerfile" -t vinted-discord-bot:3.17 "."`,
    (err, stdout, stderr) => {
      if (!err) {
        console.log(`👋 Local dependency created`);
        ...
        exec(`docker volume ls -q`, (err, stdout, stderr) => {
          ...
          bots.forEach((bot) => {
            ...
            const start = () => {
              exec(
                `BOT=${bot.name} VINTED_BOT_ADMIN_IDS=${bot.adminIDs} VINTED_BOT_TOKEN=${bot.token} docker-compose -f docker-compose.yaml -p bot-${bot.name} up -d`,
                (err, stdout, stderr) => {
                  if (err) {
            ...
            if (volumes.includes(`bot-${bot.name}`)) {
              ...
              exec(
                `docker volume create bot-${bot.name}`,
                (err, stdout, stderr) => {
                  if (!err) {
        ...
        console.log(`👎 Error creating local dependecy`);
        throw err;
      }
    }
  );
...
if (restart) {
  ...
  bots.forEach((bot) => {
    exec(`docker-compose -p bot-${bot.name} stop`, (err, stdout, stderr) => {
      if (!err) {
        exec(
          `docker-compose -p bot-${bot.name} rm -f`,
          (err, stdout, stderr) => {
            if (!err) {
